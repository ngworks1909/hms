...
export default function Discharge() {
    ...
    const discharge = async(registerId: string) => {
      const response = await fetch(`/api/dischargeuser/${registerId}`, {
        method: "UPDATE",
      });
        ...
        <div className='flex h-[50dvh] flex-col mt-4 gap-4 overflow-hidden overflow-y-scroll'>
          ...
          {users.map((user: BillType, index: number) => {
            ...
            <Avatar>
              ...
              <Button  onClick={async() => {await discharge(user.registerId)}} >Discharge</Button>
            </Card>
            </>
          })}
        </div>
